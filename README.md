...
Exponent (8 bits): Stores the exponent of the number in ”biased” form. The bias for the
exponent in single precision is 127. The actual exponent E=exponent bits−127
Mantissa (23 bits): Stores the fractional part (the significand) of the number, where the first
bit is assumed to be 1 (implicit leading 1 for normalized numbers)
